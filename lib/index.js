...
class Skeleton {
  ...
  async ensureRoot(rootDir) {
    ...
    if (!exists) {
      let answer = await question(`目录${rootDir}不存在，将创建该目录，并以此作为根目录？Y/n? `);
      if (answer === 'Y' || answer === '') {
      ...
      let answer = await question(`目录${rootDir}已存在，以此作为根目录？Y/n? `);
      if (answer === 'Y' || answer === '') {
        this.setRoot(rootDir);
      } else {
        await this.ensureRoot();
  ...
  async run() {
    ...
    for (var i = 0; i < this.files.length; i++) {
      let action = this.files[i];
      const target = path.join(this.rootDir, action.target);
      const dir = path.dirname(target);
      await mkdirp(dir);
      const sourcePath = action.source;
      const writable = fs.createReadStream(sourcePath)
        .pipe(fs.createWriteStream(target));
      await finish(writable);
    }
